...
class StateAgency(Agency):
    ...
    def get_final_costs(self):
        """Written July 21st for reportback to Bobby on a summary of costs per year using final methodology.
        This will change as we tweak methodology"""
        if self.budget_by_year.empty:
            self.add_budget_by_year()
        self.appropriations_rev_9c()
        return self.correction_function(self.budget_by_year.loc["All Appropriations Plus Revenues Minus 9c"])
    ...
    def add_payroll_by_year(self, total_OT_only):
...
class MBTA(StateAgency):
    ...
    def __init__(self, alias, official_name, year_range, category, client, correction_function):
    ...
    def get_payroll_by_year(self):
        """This is combination of cthru and budget pdf I found on line"""
...
class PoliceDepartment(Agency):
    ...
    def get_final_costs(self):
        out_df = self.budget_summary.loc[["Total Budget"]]
        out_df.loc["Total Budget"] = [out_df.iloc[0, x] if out_df.iloc[0, x] > 0 else out_df.iloc[0, x-1]
                                      for x in range(out_df.shape[1])]
        return out_df.loc["Total Budget"]
...
class BostonPD(PoliceDepartment):
    ...
    def PDF_scraper_extractable(self, year, path, payroll_signifier):
        ...
        for i in range(budget_obj.getNumPages()):
            ...
            if PD_section and payroll_signifier in budget_obj.getPage(i).extractText():
                ...
                self.update_budget_summary(year, total_dollar_amounts, "Overall")
            if PD_section and "Police Department Capital Budget" in budget_obj.getPage(i).extractText():
                ...
                self.update_budget_summary(year, capital_projects_amounts, "Overall")
                break
    ...
    def PDF_scraper_non_extractable(self, year, path, payroll_signifier):
        ...
        if year == 2018:
            ...
            for i in range(2):
                ...
                self.update_budget_summary(year, overall_dollar_amounts, "Overall")
        ...
        self.update_budget_summary(year, capital_dollar_amounts, "Overall")
    ...
    def alt_get_dollar_amounts_payroll(self, year):
        ...
        self.budget_summary.loc["Payroll Budget", year] = 337939296 + 4789220
        self.budget_summary.loc["Payroll Appropriation", year - 1] = 328663838 + 4383901
        self.budget_summary.loc["Payroll Expenditures", year - 2] = 319608659 + 4387452
        self.budget_summary.loc["OT Budget", year] = 56494667 + 1067784
        self.budget_summary.loc["OT Appropriation", year - 1] = 55660719 + 566556
        self.budget_summary.loc["OT Expenditures", year - 2] = 57479518 + 570673
        self.budget_summary.loc["Total Budget", year] = 373814105 + 11506920
        self.budget_summary.loc["Total Appropriation", year - 1] = 364321048 + 10498013
        self.budget_summary.loc["Total Expenditures", year - 2] = 348887844 + 9562319

    def update_budget_summary(self, year, dollar_amounts, line_item):
        """Updated by Sasha on July 16th to have dollar amounts add to existing values so that capital projects,
        external funds can be easily added on"""
