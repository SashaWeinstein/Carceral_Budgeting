...
class PoliceDepartment(Agency):
    ...
    def Add_True_Earnings(self):
        ...
        for year in total_earnings.index:
            ...
            self.operating_costs[year] = \
                self.budget_summary.loc["Non-Payroll Expenditures", year] + \
                self.budget_summary.loc["True Payroll", year]
            # self.budget_summary.loc["Final Expenditures", year] = \
            #     self.budget_summary.loc["Final Yearly Operating Cost", year] + self.pensions[year] + self.fringe[year]

        if 2016 not in total_earnings.index:
            self.operating_costs[2016] =\
...
class ChelseaPD(PoliceDepartment):
    ...
    def update_budget_summary_2021(self, year, dollar_amounts, line_item):
        ...
        if line_item == "Non-Capital":
            ...
            assert dollar_amounts is not None, "Dollar amounts for " + str(year) + " Chelsea PD capital expenditures not found"
...
class ReverePD(PoliceDepartment):
    ...
    def __init__(self):
        ...
        self.scrape()
        self.pensions, self.PD_fraction = ReverePD_Pensions(self.budget_summary.loc["Payroll Adopted", list(range(2016, 2020))])
    ...
    def get_final_costs(self, add_hidden_costs=True, pensions_statewide=None):
        ...
        self.operating_costs[2019] = self.budget_summary.loc["Total Budget", 2019]
        if add_hidden_costs:
            return self.operating_costs + self.pensions + self.fringe + self.capital_expenditures
        else:
...
class WinthropPD(PoliceDepartment):
    ...
    def get_final_costs(self, add_hidden_costs=True, pensions_statewide=None):
        """Written August 12th"""
        self.operating_costs = self.budget_summary.loc["Total Expenditures", list(range(2016, 2020))]
        self.operating_costs.loc[2018] = self.budget_summary.loc["Total Budget", 2018] - self.capital_expenditures.loc[2018]
